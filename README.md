# これは何？
- 画像処理100本ノックをC++で解いていっており、自身の解答を公開しています。(2025.03現在)
    - 特に51~は本家版だとC++の解答が無いため、参考になれば幸いです。
- 筆者は競技プログラミングを通じてC++を学んだ人間のため、プロダクトでC++を書く際の作法に沿えていない可能性があります。比較的愚直にアルゴリズムを実装するような形の解答になっているかと思います。
- また、Pythonでの実行時間と比較するために、時間計測用のコードをmain関数内に記載しています。
- 加えて、解いて行く過程で学習したこと・気づいたことを、各問題のREADME.mdに追記しています。
- ※yoyoyo-yo氏のリポジトリは現在公開されていないため、forkされたものを更にforkする形で作成しています。ご了承願います。
