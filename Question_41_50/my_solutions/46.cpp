...
std::vector<std::vector<int>> topk_NMS_Hough(std::vector<std::vector<int>> NMS_Hough, int K){
  int height = NMS_Hough.size();
  int width = NMS_Hough[0].size();
  std::vector<int> val;
  std::vector<std::vector<int>> out(height, std::vector<int>(width, 0));
  for (int x = 0; x < height; x++){
    for (int y = 0; y < width; y++){
  ...
  for (int x = 0; x < height; x++){
    for (int y = 0; y < width; y++){
      if (NMS_Hough[x][y] >= threshold){
        out[x][y] = NMS_Hough[x][y];
      }
    }
  }
  return out;
...
cv::Mat inverse_Hough(std::vector<std::vector<int>> NMS_Hough, cv::Mat Original){
  cv::Mat ret = Original.clone();
  int height = Original.rows;
  int width = Original.cols;
  int rmax = sqrt(height * height + width * width);
  for (int rho = 0; rho < NMS_Hough.size(); rho++){
    for (int t = 0; t < NMS_Hough[0].size(); t++){
      if (NMS_Hough[rho][t] > 0){
        // std::cout << "rho: " << rho << " theta: " << t << std::endl;
        ...
        if (sint == 0) sint += 1e-8;
        for (int y = 0; y < width; y++){
          int x = -cost / sint * y + (rho - rmax) / sint;
          // std::cout << x << " " << y << std::endl;
          if (x >= 0 && x < height){
            // std::cout << x << " " << y << std::endl;
            ret.at<cv::Vec3b>(x, y) = cv::Vec3b(0, 0, 255);
          }
        }
        for (int x = 0; x < height; x++){
          int y = -sint / cost * x + (rho - rmax) / cost;
          // std::cout << x << " " << y << std::endl;
          if (y >= 0 && y < width){
            // std::cout << x << " " << y << std::endl;
            ret.at<cv::Vec3b>(x, y) = cv::Vec3b(0, 0, 255);
...
int main(int argc, const char* argv[]){
  ...
  cv::Mat canny = Canny(nms, 100, 30);
  // ここまでcanny法

  auto vote_result = Hough_vote(canny);
  auto nms_hough = NMS_Hough(vote_result);
  // auto out = convert_vec2Mat(nms_hough);
  nms_hough = topk_NMS_Hough(nms_hough, 25); // K = 25で良い感じだった
  auto out = inverse_Hough(nms_hough, img);
