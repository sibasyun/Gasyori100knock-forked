...
cv::Mat affine(cv::Mat img, double a, double b, double c, double d, double tx, double ty, double theta){
  ...
  int resized_width = (int)(width * (a + b)); // resize後の画像サイズをa, bに基づいて変更
  int resized_height = (int)(height * (c + d)); // resize後の画像サイズをc, dに基づいて変更
...
int main(int argc, const char* argv[]){
  ...
  double a = 30. / (double) h;
  double b = 30. / (double) w;

  // affine
  cv::Mat out = affine(img, 1, a, b, 1, 0, 0, 0);
