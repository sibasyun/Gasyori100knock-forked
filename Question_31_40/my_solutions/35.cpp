...
// high path filter
std::vector<std::vector<std::complex<float>>> HPF(std::vector<std::vector<std::complex<float>>> G, float pass_ratio){
  int height = G.size();
  int width = G[0].size();
...
// band path filter
std::vector<std::vector<std::complex<float>>> BPF(std::vector<std::vector<std::complex<float>>> G, float low_pass_ratio, float high_pass_ratio){
  int height = G.size();
  int width = G[0].size();
  
  int r = height/2;
  
  int low_filter_d = (int)(float(r) * low_pass_ratio);
  int high_filter_d = (int)(float(r) * high_pass_ratio);
  for (int y = 0; y < G.size() / 2; y++){
    for (int x = 0; x < G[0].size() / 2; x++){
      if (float(y * y + x * x) <= low_filter_d * low_filter_d || float(y * y + x * x) >= high_filter_d * high_filter_d){
        G[y][x] = 0;
        G[y][width-x-1] = 0;
        G[height-y-1][x] = 0;
        G[height-y-1][width-x-1] = 0;
